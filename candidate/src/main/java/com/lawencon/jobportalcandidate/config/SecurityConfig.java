...
import com.lawencon.jobportalcandidate.filter.AuthorizationFilter;
...
public class SecurityConfig {
	...
	@Bean
	public AuthenticationManager authManager(HttpSecurity http, CandidateService candidateService, BCryptPasswordEncoder encoder)
			throws Exception {

		return http.getSharedObject(AuthenticationManagerBuilder.class).userDetailsService(candidateService)
	...
	public List<RequestMatcher> matchers() {
		...
		matchers.add(new AntPathRequestMatcher("/candidate-user", HttpMethod.POST.toString()));
	...
	public WebMvcConfigurer mvcConfigurer() {
		return new WebMvcConfigurer() {
			...
			public void addCorsMappings(CorsRegistry registry) {
				registry.addMapping("/**")
				.allowedOrigins("http://localhost:4201","http://192.168.20.30:4201")
